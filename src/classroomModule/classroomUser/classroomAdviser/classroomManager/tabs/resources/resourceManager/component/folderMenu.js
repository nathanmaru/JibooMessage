...
const FolderMenu = () => {
	...
	const handleCreateFolder = () => {
		...
		dispatch(addFolder(`/resource/classroom/folder`));
	};
	const handleEditFolder = () => {
		let formData = new FormData();
		formData.append('name', inputForm.name);
		formData.append('resource', id);
		dispatch(editFolder(`/resource/classroom/folder/change/${folder}`, formData));
	};
	...
	useEffect(() => {
		if (status == 'Folder delete success') {
			history.replace('/classroom/adviser/resources/' + id);
		}
	}, [status]);
	...
	const onSubmit = (data) => {
		...
		formData.append('resource', id);
		dispatch(addFolder(`/resource/classroom/folder`, formData));
	};
